public static class ArraySelector
{
    public static void Run()
    {
        var l1 = new[] { 1, 2, 3, 4, 5 };
        var l2 = new[] { 2, 4, 6, 8, 10};
        var select = new[] { 1, 1, 1, 2, 2, 1, 2, 2, 2, 1};
        var intResult = ListSelector(l1, l2, select);
        Console.WriteLine("<int[]>{" + string.Join(", ", intResult) + "}"); // <int[]>{1, 2, 3, 2, 4, 4, 6, 8, 10, 5}

        var l3 = new[] { 'A', 'A', 'A', 'A', 'A'};
        var l4 = new[] { 'B', 'B', 'B', 'B', 'B'};
        select = [1, 2, 1, 2, 1, 2, 1, 2, 1, 2];
        var charResult = ListSelector(l3, l4, select);
        Console.WriteLine("<char[]>{" + string.Join(", ", charResult) + "}"); // <char[]>{A, B, A, B, A, B, A, B, A, B}
    }

    private static int[] ListSelector(int[] list1, int[] list2, int[] select)
    {
        // Build result array with same length as select.
        // Maintain separate indices for list1 and list2.
        // For each entry in select:
        //   - if select[i] == 1, take the next element from list1
        //   - if select[i] == 2, take the next element from list2
        var result = new int[select.Length];
        int index1 = 0;
        int index2 = 0;

        for (int i = 0; i < select.Length; i++)
        {
            if (select[i] == 1)
                result[i] = list1[index1++];
            else
                result[i] = list2[index2++];
        }

        return result;
    }

    private static char[] ListSelector(char[] list1, char[] list2, int[] select)
    {
        // Same logic as the int version, but operates on char arrays.
        // Uses the ternary operator for a more concise implementation.
        var result = new char[select.Length];
        int index1 = 0;
        int index2 = 0;

        for (int i = 0; i < select.Length; i++)
        {
            result[i] = select[i] == 1 ? list1[index1++] : list2[index2++];
        }

        return result;
    }
}